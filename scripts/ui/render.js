...
function isManualTranslation(translation) {
  return Boolean(translation && (translation.origin === 'manual' || !translation.promptId));
}
...
function displayedVersionCount(segment) {
  return segment.translations.length + segment.translations.filter((item) => item.aiPostEdit).length;
}
...
function allVersionTargets(state, project, segment, index) {
  ...
  const cards = versions.map((translation) => {
    ...
    if (!translation.aiPostEdit) return regular;
    const aiVersion = aiPostEditVersionTarget(project, segment, index, translation);
...
function versionTarget(state, project, segment, index, translation) {
  ...
  const hasAiVersion = Boolean(translation.aiPostEdit);
...
function renderStatus(state) {
  const segment = store.getSegment();
  document.querySelector('#save-status').textContent = segment?.status === 'ai-edited'
    ? 'AI已编辑 · 建议与决策已保存在本机' : '所有更改保存在本机';
  document.querySelector('#current-user-status').textContent = state.role === 'teacher'
    ? '当前：林老师（教师）' : '当前：当前学生（学生）';
}
