 ...
 * 依赖内部: ../auth/authorization.ts, ../context.ts, ./project-resources.ts, ./project-snapshot.ts, ./projects.ts
 * 依赖外部: fastify
...
import { currentProjectBrief, generateProjectBrief, generateProjectPrompt, listProjectResourceCatalog, saveProjectBrief,
  type BriefContent } from './project-resources.js';
import { buildProjectSnapshot } from './project-snapshot.js';
import { assignProject, createProject, createWorkspace, deleteProject, listVisibleProjects, openProjectWorkspace,
  projectDetail, publishProject, unpublishProject, type ProjectInput } from './projects.js';
...
interface GenerateBody { modelConfigId?: string }
...
function registerProjectWriteRoutes(app: FastifyInstance, context: AppContext): void {
  ...
  app.post<{ Params: ProjectParams; Body: GenerateBody }>('/api/projects/:projectId/brief/generate',
    { preHandler: requireReadyAccount }, async (request, reply) => reply.code(201).send({
      id: await generateProjectBrief(context, request.authUser!, request.params.projectId,
        request.body?.modelConfigId),
    }));
  app.post<{ Params: ProjectParams; Body: GenerateBody }>('/api/projects/:projectId/prompt/generate',
    { preHandler: requireReadyAccount }, async (request, reply) => reply.code(201).send({
      id: await generateProjectPrompt(context, request.authUser!, request.params.projectId,
        request.body?.modelConfigId),
    }));
